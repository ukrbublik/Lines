...
#ifdef __COMPATIBLE_WITH_BUTTONS__	
	m_hwndBtn = NULL;
#endif // __COMPATIBLE_WITH_BUTTONS__
...
// 	#ifdef __COMPATIBLE_WITH_BUTTONS__
// 		POINT pt_client = { pMsg->pt.x, pMsg->pt.y };
...
//		if (!(m_tiDisplayed.nBehaviour & PPTOOLTIP_NOCLOSE_MOUSEDOWN))
	...
	#ifdef __COMPATIBLE_WITH_BUTTONS__
		if(pMsg->message == WM_LBUTTONDOWN && WindowFromPoint(pMsg->pt)->IsKindOf(RUNTIME_CLASS(CButton)))
		{
			#ifdef _DEBUG
				Beep(250, 1);
			#endif // _DEBUG
			...
			if(m_hwndBtn)
			{
				((CButtonST*)FromHandle(m_hwndBtn))->m_bIsCoveredByToolTip = FALSE;
				m_hwndBtn = NULL;
			}

			POINT pt_client = { pMsg->pt.x, pMsg->pt.y };
			::ScreenToClient(WindowFromPoint(pMsg->pt)->GetSafeHwnd(), &pt_client);
			::SendMessage(WindowFromPoint(pMsg->pt)->GetSafeHwnd(), WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(pt_client.x, pt_client.y));
		}
	#endif // __COMPATIBLE_WITH_BUTTONS__
		...
		else if (PPTOOLTIP_STATE_HIDEN == m_nTooltipState)
		{
			...
			m_nTooltipState = PPTOOLTIP_STATE_SHOWING;
				...
				if(!m_hwndBtn)
				{
					POINT pt;
					GetCursorPos(&pt);
					m_hwndBtn = WindowFromPoint(pt)->GetSafeHwnd();
					if(m_hwndBtn)
					{
						((CButtonST*)FromHandle(m_hwndBtn))->m_bIsCoveredByToolTip = TRUE;
						TRACE(_T("<BTN INVALIDATE>:  + %d\n"), WindowFromPoint(pt)->GetDlgCtrlID());
					}
				}
			#endif // __COMPATIBLE_WITH_BUTTONS__
	...
	case TIMER_HIDING:
		...
		if (PERCENT_MAX_TRANSPARENCY == m_dwCurTransparency)
		{
			...
			m_nTooltipState = PPTOOLTIP_STATE_HIDEN;
				...
				if(m_hwndBtn)
				{
					FromHandle(m_hwndBtn)->Invalidate();
					TRACE(_T("<BTN INVALIDATE>:  - %d\n"), FromHandle(m_hwndBtn)->GetDlgCtrlID());
